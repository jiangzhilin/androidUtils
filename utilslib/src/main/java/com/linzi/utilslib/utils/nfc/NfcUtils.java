...
//            Parcelable[] rawArray = intent.getParcelableArrayExtra(NfcAdapter.EXTRA_NDEF_MESSAGES);
            ...
            if (rawArray != null) {
                ...
                Log.e("原始数据", "readNFCFromTag: "+ByteArrayToHexString(mNdefRecord.getPayload()) );
                /**
                 * @author linzi
                 * @date 2019/11/20 9:31
                 * @context 要去除前六位包头才是真正的ndef信息
                 */
                if (mNdefRecord != null) {
                    byte[] data=mNdefRecord.getPayload();
//                    byte[] data=mNdefRecord.getType();
                    byte[] result=new byte[data.length-6];
                    for(int x=0;x<data.length-6;x++){
                        result[x]=data[x+6];
                    }
                    String readResult = new String(data, "UTF-8");

                    if(readResult.contains("zh")){
                        readResult=readResult.substring(readResult.indexOf("zh")+"zh".length(),readResult.length());
                    }else if(readResult.contains("utf-8")){
                        readResult=readResult.substring(readResult.indexOf("utf-8")+"utf-8".length(),readResult.length());
                    }else if(readResult.contains("en")){
                        readResult=readResult.substring(readResult.indexOf("en")+"en".length(),readResult.length());
                    }
...
//            String id = new String(tag.getId());
//            String id = HexUtils.ByteArray2String(tag.getId());
            return id;
        }
